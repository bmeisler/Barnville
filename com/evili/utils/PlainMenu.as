package com.evili.utils
{
	...
	public class PlainMenu extends Sprite
	{
		...
		public function PlainMenu(template:Object)
		{
		...
		private function createIcons():void
		{
			...
			for ( var i:Number = 1; i < items.length + 1; i++ )
			{
				...
				itemLabel.text = items[i-1].price;
				itemLabel.setTextFormat(myformat);
				///container.addChild(itemLabel);
				addChild(itemLabel);
				
				//NOTE$$$ cont is the movieclip that contains the icon
				...
				///cont.y = -icon_spacing;
				//ADJUSTING Y LOCATION OF ICON
				cont.y = -icon_spacing - 17;
				...
				//AWFUL HACK, POSITIONING LABELS BASED ON NAME...but I'm tired
				var currentName:String = items[i-1].name;
				
				if (currentName.indexOf("Left") == -1){
					itemLabel.x += 10;
				}
				if (currentName == "Cat" ||currentName == "Dog" || currentName == "Chicken" || currentName == "Dog" || currentName == "Pig"){
					itemLabel.x -= 5;
				}
				//ADJUSTING Y LOCATION OF ICON'S LABEL
				itemLabel.y = -icon_spacing-22;
		...
		private function onClick(event:MouseEvent):void
		{
			...
			trace("d: "  + d);
				...
				clickedItem = items[event.target.parent.name - 1].name
		...
		/** sets the x, y position and width and height of tray - NOT done above */
		private function updateTray():void
		{
			...
			getChildByName("tray").y -= getChildByName("tray").y + 10;
			///getChildByName("tray").width = w - x;
			//NOTE$$$ magic number (25) added on end - otherwise tray was always too short
			getChildByName("tray").width = w - x + 25;
		}
	}
}